...
>(
  (
      ...
      const Item = (
        <CommandItem
          ...
          onSelect={(value: string) => {
            ...
            setInputValue("");
            const newOptions = [...selected, { value, text: value }];
            setSelected(newOptions);
            onChange?.(newOptions);
          }}
    ...
    const arr = Object.entries(selectables);
    ...
    return (
      ...
      >
        <div
          className={cn(
            "group rounded-md border border-border px-3 py-2 bg-white text-sm ",
            className
          )}
        >
        ...
        <div className="relative mt-2">
          {open && (
            <CommandList className="absolute top-0 z-10 w-full rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in">
              {isLoading ? (
                <div className="relative h-[50px]">{loadingIndicator}</div>
              ) : (
                <>
                  {EmptyItem()}
                  {CreatableItem()}
